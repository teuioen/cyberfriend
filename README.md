...
**GitHub**: [github.com/teuioen](https://github.com/teuioen/)
...
| **健康系统** | 健康值/疲惫值，随机患病（感冒/抑郁/焦虑），积极情绪促进康复 |
...
| **虚拟货币** | 用户每日系统发放，可赠礼/购物互动，可打工赚钱 |
| **心跳系统** | 后台轮询（默认30分钟），自主决策：联系用户/写日记/查看日记/工作/睡觉/无动作 |
...
| `/memories` | 查看所有记忆（`/memories search 关键词` 搜索特定记忆） |
| `/dreams` | 查看梦境记录 |
...
AI 通过在回复中嵌入标签来控制内部状态。

以下为系统提示词中使用的行动标签规范（与内置解析器/执行器保持一致）：

```
<!-- 调整情绪（每次回复都必须用，支持 9 个维度：joy/sadness/anxiety/anger/fear/excitement/disgust/shame/curiosity） -->
<EMOTION joy="±N" anxiety="±N" curiosity="±N" />

<!-- 保存记忆（原 MEMORY_SAVE，两个都支持） -->
<MEMORY_ADD level="permanent" importance="1-10">[内容]</MEMORY_ADD>

<!-- 更新记忆 -->
<MEMORY_UPDATE id="N" importance="N">[内容]</MEMORY_UPDATE>
...
<!-- 写日记 -->
<DIARY_WRITE>[日记内容]</DIARY_WRITE>

<!-- 翻阅日记（random=true随机，默认最近几篇） -->
<DIARY_READ limit="3" random="true" />

<!-- 开始睡觉N小时 -->
<SLEEP duration="7" />

<!-- 心跳时主动联系用户 -->
<SEND_MESSAGE>[消息内容]</SEND_MESSAGE>

<!-- 创建定时任务 -->
<TASK_CREATE name="任务名" trigger_time="2025-06-01 08:00">[描述]</TASK_CREATE>

<!-- 调整下次心跳间隔 -->
<NEXT_HEARTBEAT minutes="60" />

<!-- 为自己购买物品 -->
<SHOP_BUY item="物品" qty="1" />
...
消息用 `|` 分割成多条，AI自动随机延迟发送，模拟真实聊天节奏：

```
哎，你今天怎么样？|最近我在看一部很有意思的剧~|你猜猜是什么题材的？
```
